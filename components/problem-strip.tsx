      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {problems.map(({ icon: Icon, title, body, tone, chip }) => (
            <div
              key={title}
              className="rounded-2xl border border-slate-200/80 bg-white/70 p-7 shadow-sm backdrop-blur transition hover:-translate-y-1 hover:shadow-lg hover:shadow-violet/10"
            >
              ...
              <span className="mt-4 inline-block rounded-full bg-violet/10 px-3 py-1 text-xs font-semibold text-violet">
                {chip}
              </span>
            </div>
          ))}
